...
const VERDICT_STYLES = {
  SMASH:       { bg: "#edf5f0", accent: "#1e6b4a", glow: "0 0 40px rgba(30,107,74,0.2)" },
  "YES, IF":   { bg: "#f5f0e5", accent: "#9a6e1a", glow: "0 0 40px rgba(154,110,26,0.2)" },
  INVESTIGATE: { bg: "#edf0f7", accent: "#1a3354", glow: "0 0 40px rgba(26,51,84,0.2)" },
  PASS:        { bg: "#f5ede8", accent: "#b5451b", glow: "0 0 40px rgba(181,69,27,0.2)" },
};
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f0e8", fontFamily: "'IBM Plex Mono','Courier New',monospace", color: "#1a1a1a", fontSize: bigType ? "18px" : "14px" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@300;400;500;600&family=Bebas+Neue&display=swap');
        * { box-sizing: border-box; }
        .scan-btn { background: #1a3354; color: #f5f0e8; border: none; padding: 14px 36px; font-family: 'IBM Plex Mono',monospace; font-weight: 600; font-size: 0.93em; letter-spacing: 2px; text-transform: uppercase; cursor: pointer; transition: all 0.15s; }
        .scan-btn:hover { background: #0f2040; transform: translateY(-1px); }
        .scan-btn:disabled { opacity: 0.4; cursor: default; transform: none; }
        .reset-btn { background: transparent; color: #5a5550; border: 1px solid #b0a898; padding: 10px 24px; font-family: 'IBM Plex Mono',monospace; font-size: 0.79em; letter-spacing: 2px; text-transform: uppercase; cursor: pointer; transition: all 0.15s; }
        .reset-btn:hover { color: #1a1a1a; border-color: #5a5550; }
        .type-toggle { background: transparent; color: #5a5550; border: 1px solid #b0a898; padding: 6px 14px; font-family: 'IBM Plex Mono',monospace; font-size: 0.79em; letter-spacing: 1px; cursor: pointer; transition: all 0.15s; margin-left: auto; }
        .type-toggle:hover { color: #1a1a1a; border-color: #5a5550; }
        .type-toggle.active { background: #1a3354; color: #f5f0e8; border-color: #1a3354; }
        textarea { width: 100%; background: #ede8de; border: 1px solid #c0b8ae; color: #1a1a1a; font-family: 'IBM Plex Mono',monospace; font-size: 1em; line-height: 1.7; padding: 20px; resize: vertical; outline: none; transition: border-color 0.2s; }
        textarea:focus { border-color: #1a3354; }
        textarea::placeholder { color: #9a9490; }
        .tag { display: inline-block; padding: 4px 10px; font-size: 0.71em; letter-spacing: 1.5px; text-transform: uppercase; border: 1px solid; margin: 3px; }
        .reveal { animation: reveal 0.35s ease-out; }
        @keyframes reveal { from { opacity:0; transform:translateY(6px); } to { opacity:1; transform:translateY(0); } }
        .match-block { border-left: 3px solid; padding: 14px 16px; margin-bottom: 10px; background: rgba(0,0,0,0.03); }
        .sentence-quote { font-size: 0.86em; color: #4a4a4a; line-height: 1.7; margin-top: 8px; padding: 10px 14px; background: #e8e2d8; border-left: 2px solid #c0b8ae; font-style: italic; }
        .negated-badge { display: inline-block; font-size: 0.64em; letter-spacing: 1.5px; padding: 2px 7px; border: 1px solid #b0a898; color: #6b6560; margin-left: 8px; vertical-align: middle; }
      `}</style>

      <div style={{ borderBottom: "1px solid #d0c9be", padding: "24px 40px", display: "flex", alignItems: "center", gap: "16px" }}>
        <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: "2em", letterSpacing: "4px" }}>JOB POSTING REALITY CHECK</div>
        <div style={{ color: "#4a5a7a", fontSize: "0.79em", letterSpacing: "2px" }}>SMASH / INVESTIGATE / PASS</div>
        <button className={`type-toggle${bigType ? " active" : ""}`} onClick={() => setBigType(b => !b)}>
          {bigType ? "Aa−" : "Aa+"}
        </button>
      </div>
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto", padding: "40px" }}>
        {!result ? (
          <>
            <div style={{ border: "1px solid #d0c9be", padding: "20px 24px", marginBottom: "32px", background: "#ede8de" }}>
              <div style={{ fontSize: "0.71em", letterSpacing: "2px", color: "#5a5550", marginBottom: "12px" }}>PRIME DIRECTIVE</div>
              <div style={{ fontSize: "0.86em", color: "#3a3a3a", lineHeight: "1.8" }}>
                "Yes, if the cash compensates me for the risk of exit."<br/>
                <span style={{ color: "#6b6b6b" }}>Threshold → Hourly: $85–100+ &nbsp;|&nbsp; W2: $150k+ &nbsp;|&nbsp; Exit window: 6 months</span>
              </div>
            </div>
            <div style={{ marginBottom: "8px", fontSize: "0.71em", letterSpacing: "2px", color: "#5a5550" }}>PASTE JOB POSTING BELOW</div>
          ...
          <div className="reveal">
            ...
            <div style={{ background: vs.bg, border: `1px solid ${vs.accent}`, boxShadow: vs.glow, padding: "32px", marginBottom: "32px", textAlign: "center" }}>
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: "4.5em", letterSpacing: "8px", color: vs.accent, lineHeight: 1, marginBottom: "12px" }}>{result.verdict}</div>
              <div style={{ fontSize: "0.93em", color: "#999", letterSpacing: "1px" }}>{result.sub}</div>
            </div>

            {/* Stats */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "10px", marginBottom: "24px" }}>
              {[
                { label: "Red Flags", value: result.redCount, color: "#b5451b" },
                { label: "Investigate", value: result.yellowCount, color: "#9a6e1a" },
                { label: "Negated*", value: result.negatedCount, color: "#9a9490" },
                { label: "Unicorn Signals", value: result.unicornFound.length, color: "#1e6b4a" },
              ].map(s => (
                <div key={s.label} style={{ background: "#ede8de", border: "1px solid #d0c9be", padding: "16px", textAlign: "center" }}>
                  <div style={{ fontSize: "2.4em", fontFamily: "'Bebas Neue',sans-serif", color: s.color, letterSpacing: "2px" }}>{s.value}</div>
                  <div style={{ fontSize: "0.64em", letterSpacing: "2px", color: "#5a5550", marginTop: "4px" }}>{s.label.toUpperCase()}</div>
                </div>
              ))}
            </div>

            {result.negatedCount > 0 && (
              <div style={{ fontSize: "0.71em", color: "#6b6b6b", marginBottom: "24px", letterSpacing: "1px" }}>
                * Negated matches score at half weight. Still shown below — human call required.
              </div>
            )}

            {/* Compensation */}
            <div style={{ border: "1px solid #d0c9be", padding: "16px 20px", marginBottom: "32px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#ede8de" }}>
              <div style={{ fontSize: "0.71em", letterSpacing: "2px", color: "#5a5550" }}>COMPENSATION</div>
              <div style={{ fontSize: "0.93em", letterSpacing: "0.5px", color: result.rateAssessment.color === "green" ? "#1e6b4a" : result.rateAssessment.color === "yellow" ? "#9a6e1a" : result.rateAssessment.color === "red" ? "#b5451b" : "#9a9490" }}>
            ...
            {result.flagResults.length > 0 && (
              <div style={{ marginBottom: "32px" }}>
                <div style={{ fontSize: "0.71em", letterSpacing: "2px", color: "#5a5550", marginBottom: "16px" }}>
                ...
                {result.flagResults.map(({ flag, matches }) => (
                  <div key={flag.id} style={{ marginBottom: "24px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                      <div style={{ fontSize: "0.79em", fontWeight: "600", letterSpacing: "1px", color: flag.risk === "red" ? "#b5451b" : "#9a6e1a" }}>{flag.label}</div>
                      <div style={{ fontSize: "0.71em", color: "#5a5550", letterSpacing: "1px" }}>{flag.risk === "red" ? "🔴 PASS signal" : "🟡 INVESTIGATE"}</div>
                    </div>
                    <div style={{ fontSize: "0.79em", color: "#5a5550", marginBottom: "10px", lineHeight: "1.5" }}>{flag.translation}</div>
                    {matches.map((m, i) => (
                      <div key={i} className="match-block" style={{ borderLeftColor: m.negated ? "#c0b8ae" : flag.risk === "red" ? "#b5451b" : "#9a6e1a" }}>
                        <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: "6px", marginBottom: "8px" }}>
                          <span style={{ fontSize: "0.71em", letterSpacing: "1.5px", color: m.negated ? "#6b6560" : flag.risk === "red" ? "#b5451b" : "#9a6e1a" }}>
                            MATCHED: "{m.phrase}"
                          </span>
                        ...
                        {m.negated && (
                          <div style={{ fontSize: "0.71em", color: "#6b6560", marginTop: "8px", letterSpacing: "0.5px" }}>
            ...
            {result.unicornFound.length > 0 && (
              <div style={{ marginBottom: "32px" }}>
                <div style={{ fontSize: "0.71em", letterSpacing: "2px", color: "#5a5550", marginBottom: "12px" }}>UNICORN SIGNALS FOUND</div>
                {result.unicornFound.map(s => (
                  <span key={s.phrase} className="tag" style={{ color: "#1e6b4a", borderColor: "#2d8a62" }}>{s.label}</span>
                ))}
              </div>
            )}

            {/* Decision rules */}
            <div style={{ border: "1px solid #d0c9be", background: "#ede8de", padding: "20px 24px", marginBottom: "32px" }}>
              <div style={{ fontSize: "0.71em", letterSpacing: "2px", color: "#5a5550", marginBottom: "14px" }}>DECISION RULES</div>
              {[
                ...
                <div key={i} style={{ fontSize: "0.86em", color: "#2a2a2a", lineHeight: "1.7", borderBottom: i < 2 ? "1px solid #d0c9be" : "none", paddingBottom: i < 2 ? "10px" : "0", marginBottom: i < 2 ? "10px" : "0" }}>
                  <span style={{ color: "#6b6b6b", marginRight: "12px" }}>{String(i+1).padStart(2,"0")}</span>{r}
                </div>
              ))}
            </div>
        ...
        <div style={{ marginTop: "48px", paddingTop: "24px", borderTop: "1px solid #d0c9be", fontSize: "0.71em", color: "#5a5550", letterSpacing: "2px", display: "flex", justifyContent: "space-between" }}>
